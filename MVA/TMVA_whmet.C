...
void TMVA_whmet( int train_region = 0 )
{
  ...
  TString signal_name = "TChiWH";

  //-----------------------------------------------------
  // define event selection (store in TCut sel)
  //-----------------------------------------------------
    TCut whsig("mini_whsig==1");
    TCut njets2("mini_njets == 2 && mini_njets_fwd == 0");
    TCut lowdm("mini_mchargino - mini_mlsp <= 200");
    TCut highdm("mini_mchargino - mini_mlsp > 200");
    ...
    TCut sigsel = sel;
    if (train_region == 1) sigsel += lowdm;
    else if (train_region == 2) sigsel += highdm;
    else if (train_region != 0) {
      cout << "ERROR: didn't recognize signal selection: " << train_region << endl;
      exit();
    }

    // TCut mlspGT0("ml > 0"); // The LSP = 0 is bad for madgraph.
    // can change mass selections here for signal based on SR
    TCut selSignal = sigsel;
    TCut selBackground = sel;

    cout << "Using signal selection      : " << selSignal.GetTitle() << endl;
    cout << "Using background selection      : " << selBackground.GetTitle() << endl;
  ...
  int n_backgrounds = 11;

  TString backgrounds[] = {"ttdl_lmgtau", "ttsl_lmgtau", "w1to4jets", "wz2qlnujets", "tW_lepsl", "tW_lepdl", "triboson", "diboson", "ttV", "DY1to4Jtot", "wbbjets", "whbb" };

  TString bkgPath = "/tas01/disk03/olivito/whmet/output_V00-02-24_2012_2jskim/Minibabies_V00-04-01/";

  TChain* chBackground = new TChain("t");
 
  for (int i = 0; i < n_backgrounds; i++) {
     TString backgroundChain = bkgPath + "/" + backgrounds[i] + "_whmet.root";
  ...
  TString signalPath = "/tas01/disk03/olivito/whmet/output_V00-02-36_2012_2jskim/Minibabies_V00-04-01/";

  TChain *chSignal = new TChain("t");

  //  TString base_name = signalPath + "/" + signal_name + "_" + s_train_region;
  TString base_name = signalPath + "/" + signal_name;
  TString signalChain  = base_name + "_whmet.root" ;
...
;
   if( mvaVar[ "pt_b"          ]  == 1 ) factory->AddVariable( "mini_pt_b"  ,       "P_{T}(b)" , "GeV"   , 'F' );
   if( mvaVar[ "pt_J1"          ]  == 1 ) factory->AddVariable( "mini_pt_J1"  ,       "P_{T}(jet1)" , "GeV"  , 'F' );
   if( mvaVar[ "pt_J2"          ]  == 1 ) factory->AddVariable( "mini_pt_J2"  ,       "P_{T}(jet2)" , "GeV"  , 'F' );
   if( mvaVar[ "dRleptB1"      ]  == 1 ) factory->AddVariable( "mini_dRleptB1"  ,       "#DeltaR(l,b_{1})", ""   , 'F' );
   if( mvaVar[ "bbpt"      ]  == 1 ) factory->AddVariable( "mini_bbpt"  ,       "P_{T}(bb)" , "GeV"  , 'F' );
   if( mvaVar[ "wpt"      ]  == 1 ) factory->AddVariable( "mini_wpt"  ,       "P_{T}(W)" , "GeV"  , 'F' );
   if( mvaVar[ "bbwdphi"      ]  == 1 ) factory->AddVariable( "mini_bbwdphi"  ,       "#Delta#phi(bb,W)", ""   , 'F' );
   if( mvaVar[ "bbdeta"      ]  == 1 ) factory->AddVariable( "mini_bbdeta"  ,       "#Delta#eta(b,#bar{b})", ""   , 'F' );
   if( mvaVar[ "bbdR"      ]  == 1 ) factory->AddVariable( "mini_bbdR"  ,       "#DeltaR(b,#bar{b})", ""   , 'F' );
   if( mvaVar[ "lepmetdphi"      ]  == 1 ) factory->AddVariable( "mini_lepmetdphi"  ,       "#Delta#phi(lep,E_{T}^{miss})", ""   , 'F' );
...
//   factory->SetSignalWeightExpression    ("mini_weight");
   factory->SetSignalWeightExpression    ("(mini_whweight/mini_xsecsusy)*mini_btagsf*mini_sltrigeff*mini_isrweight");
   factory->SetBackgroundWeightExpression("mini_whweight*mini_btagsf*mini_topptweight*mini_sltrigeff*mini_wbbmtcor*mini_isrweight");
