...
void AfbUnfoldTests(Int_t iVar = 0, TString TestType = "Linearity", /*Int_t slopeOption = 0,*/ Int_t Nfunction = 0)
{
    ...
    TH1D *hMeas_after = new TH1D ("measAfterScaling", "Measured", nbinsx_reco_3ch, recobins_3ch);
    TH1D *hMeas_after_combined = new TH1D ("measAfterScaling_combined", "Measured", nbinsx_reco, recobins);

    TH1D *hSmeared = new TH1D ("smeared", "Smeared", nbinsx_reco_3ch, recobins_3ch);
    ...
    for (int k = -1; k < Nlin; k++)
    {
        ...
        for (Int_t i = 0; i < entries; i++)
        {
            ...
            if ( iVar<2 || iVar==9 || ttmass > 0 )
            {
			  ...
			  fillUnderOverFlow(hMeas_after, observable+offset, weight, Nsolns);
			  fillUnderOverFlow(hMeas_after_combined, observable, weight, Nsolns);
			  ...
			  if ( combineLepMinus )
              {
				...
				fillUnderOverFlow(hMeas_after, observableMinus+offset, weight, Nsolns);
				fillUnderOverFlow(hMeas_after_combined, observableMinus, weight, Nsolns);
		  ...
		  for (int i = 0; i < nPseudos; i++)
            {
			  ...
			  */
			  ...
			  for (int iD = 1; iD < nbinsx_gen+1; ++iD)
                {
				  AfbPull[iD]->Fill( (hUnfolded_clone->GetBinContent(iD) - hTrue_after_clone->GetBinContent(iD)) / hUnfolded_clone->GetBinError(iD) );
				  SumAsym[iD] += hUnfolded_clone->GetBinContent(iD);
				  SumErrAsym[iD] += hUnfolded_clone->GetBinError(iD);
				  SumTrueAsym[iD] += hTrue_after_clone->GetBinContent(iD);
				  //SumTrueErrAsym[iD] + = hTrue_after_clone->GetBinError(iD);
