    ...
    for (Int_t iVar = 0; iVar < nVars; iVar++)
	  {
		...
		//acceptM[0]->Divide( accDen[0] );
		  ...
		  acceptMcorrected[0] = (TH1D*)(acceptM[0]->Clone("acceptMcorrected0"));
		  acceptMcorrected[1] = (TH1D*)(acceptM[1]->Clone("acceptMcorrected1"));
		  acceptMcorrected[2] = (TH1D*)(acceptM[2]->Clone("acceptMcorrected2"));
		  acceptMcorrected[3] = (TH1D*)(acceptNumcorrected->Clone("acceptMcorrected"));
		...
		if( iChan==nSig ) {     // Combined channels...
		  ...
		  acceptMcorrected[nSig]->Divide( acceptDencorrected );
		  ...
		  for( int acceptbin=1; acceptbin<=nbinsx_gen; acceptbin++ ) {
		    double acceptance = acceptMcorrected[nSig]->GetBinContent(acceptbin);
			...
			if (acceptM[iChan]->GetBinContent(i) != 0)
			  {
				...
				hData_unfolded->SetBinContent(i, hData_unfolded->GetBinContent(i) * 1.0 / acceptMcorrected[iChan]->GetBinContent(i));
				hData_unfolded->SetBinError (i, hData_unfolded->GetBinError(i) * 1.0 / acceptMcorrected[iChan]->GetBinContent(i));
			  }
        ...
        //calculate covariance matrix for corrected distribution
        for (int l = 0; l < nbinsx_gen; l++)
		  {
            for (int j = 0; j < nbinsx_gen; j++)
			  {
                m_correctE(l, j) = m_correctE(l, j) / acceptMcorrected[iChan]->GetBinContent(l+1)/acceptMcorrected[iChan]->GetBinContent(j+1);
                m_smearingE(l, j) = m_smearingE(l, j) / acceptMcorrected[iChan]->GetBinContent(l+1)/acceptMcorrected[iChan]->GetBinContent(j+1);
			  }
		  }
