    ...
    for (Int_t iVar = 0; iVar < nVars; iVar++)
	  {
        ...
        TH2D *hTrueBias = new TH2D ("trueBias", "Truth reweighted for bias",    nbinsx_gen, genbins, nbinsy2D, ybins2D);
        ...
        TH1D *hTrueBias_unwrapped = new TH1D ("trueBias_unwr", "Truth reweighted for bias unwrapped",  nbinsunwrapped_gen, 0.5, double(nbinsunwrapped_gen)+0.5);
		...
		double histmin2D = ybins2D[0];
		double histmax2D = ybins2D[3];
		cout<<"ybins: "<<histmin2D<<" "<<histmax2D<<endl;
            ...
            if ( tmass > 0 )
			  {
				...
				fillUnderOverFlow(hTrueBias, observable_gen, obs2D_gen, weight*ratio, Nsolns);
				fillUnderOverFlow(hTrue_split, observable_gen+offset, obs2D_gen, weight, Nsolns);
				if (reweightbiasonly) fillUnderOverFlow(hTrue_vs_Meas, measbin, genbin, weight, Nsolns);
				else fillUnderOverFlow(hTrue_vs_Meas, measbin, genbin, weight*ratio, Nsolns);
				...
				if ( combineLepMinus )
				  {
					...
					fillUnderOverFlow(hTrueBias, observableMinus_gen, obs2D_gen, weight*ratioMinus, Nsolns);
					fillUnderOverFlow(hTrue_split, observableMinus_gen+offset, obs2D_gen, weight, Nsolns);
					if (reweightbiasonly) fillUnderOverFlow(hTrue_vs_Meas, measbin, genbin, weight, Nsolns);
					else fillUnderOverFlow(hTrue_vs_Meas, measbin, genbin, weight*ratioMinus, Nsolns);
				  }
			  }
		  }
		...
		if( iChan==nSig ) {     // Combined channels...
		  ...
		  for( int yBin=1; yBin<=nbinsy2D; yBin++ ) {
			for( int xBin=1; xBin<=nbinsx_gen; xBin++ ) {
			  ...
			  double n_accepted = 0.;
			  if (reweightbiasonly) n_accepted = hTrue->GetBinContent( xBin, yBin );
			  else n_accepted = hTrueBias->GetBinContent( xBin, yBin );
...
*/
        ...
        for (Int_t x = 1; x <= nbinsx_gen; x++) {
		  for (Int_t y = 1; y<= nbinsy2D; y++) {
			if (acceptM[iChan]->GetBinContent(x,y) != 0) {
			  hTrueBias->SetBinContent(x,y, hTrueBias->GetBinContent(x,y) * 1.0 / acceptM[iChan]->GetBinContent(x,y));
			  hTrueBias->SetBinError  (x,y, hTrueBias->GetBinError(x,y)  * 1.0 / acceptM[iChan]->GetBinContent(x,y));
			}
		  }
		}
		...
		unwrap2dhisto(hBkg,  hBkg_unwrapped);
		unwrap2dhisto(hTrue, hTrue_unwrapped);
		unwrap2dhisto(hTrueBias, hTrueBias_unwrapped);
		unwrap2dhisto(hMeas, hMeas_unwrapped);
		...
		cout<<"****Normalisation check after A: **** "<<hTrueBias->Integral() / hTrue->Integral()<<" "<<hTrueBias_unwrapped->Integral() / hTrue_unwrapped->Integral()<<endl;
		...
		unfold_TUnfold.SetBias(hTrueBias_unwrapped);
