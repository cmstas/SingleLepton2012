...
void AfbUnfoldExample(TString Var2D = "mtt", double scalettdil = 1., double scalefake = 2.18495, double scalewjets = 1., double scaleDYeemm = 1.35973, double scaleDYtautau = 1.17793, double scaletw = 1., double scaleVV = 1.)
{
  ...
  if (Var2D == "mtt") summary_name = "summary_2Dunfolding_ratios";
  else if (Var2D == "ttrapidity2") summary_name = "summary_2Dunfolding_ttrapidity2_ratios";
  else if (Var2D == "ttpt") summary_name = "summary_2Dunfolding_ttpt_ratios";
  ...
  for (Int_t iVar = 0; iVar < nVars; iVar++)
	{
	  ...
	  // TH2D *hData_bkgSub_split_unweighted = new TH2D ("Data_bkgSub_split_unweighted", "Data_split", nbinsx_reco_split_ch, recobins_split_ch, nbinsy2D, ybins2D);
	  ...
	  // TH2D *hTrue_unweighted = new TH2D ("true", "Truth",    nbinsx_gen, genbins, nbinsy2D, ybins2D);
	  // TH2D *hTrue_split_unweighted = new TH2D ("true_split", "Truth",    nbinsx_reco_split_ch, recobins_split_ch, nbinsy2D, ybins2D);
	  ...
	  TH2D *hRatio = (TH2D*)(ratiofile->Get(acceptanceName +"_"+ Var2D +"_ratio_2d"));
		  ...
		  if ( tmass > 0 )
            {
			  ...
			  // fillUnderOverFlow(hTrue_unweighted, observable_gen, obs2D_gen, weight, Nsolns);
			  // fillUnderOverFlow(hTrue_split_unweighted, observable_gen+offset, obs2D_gen, weight, Nsolns);
			  fillUnderOverFlow(hTrue_vs_Meas, measbin, genbin, weight*ratio, Nsolns);

			  // fillUnderOverFlow(hData, observable, obs2D, weight*ratio, Nsolns);
			  // fillUnderOverFlow(hData_split, observable+offset, obs2D, weight*ratio, Nsolns);
			  ...
			  if ( combineLepMinus )
                {
				  ...
				  // fillUnderOverFlow(hTrue_unweighted, observableMinus_gen, obs2D_gen, weight, Nsolns);
				  // fillUnderOverFlow(hTrue_split_unweighted, observableMinus_gen+offset, obs2D_gen, weight, Nsolns);
				  fillUnderOverFlow(hTrue_vs_Meas, measbin, genbin, weight*ratioMinus, Nsolns);

				  // fillUnderOverFlow(hData, observableMinus, obs2D, weight*ratio, Nsolns);
				  // fillUnderOverFlow(hData_split, observableMinus+offset, obs2D, weight*ratio, Nsolns);
                }
            }
        }
	  ...
	  m_rhoMin->Draw();
	  c_rhoAvg->SaveAs("2D_" + acceptanceName + "_" + Var2D + "_minRho_KITratio.pdf");
	  ...
	  c_data->SaveAs("2D_unfolded_" + acceptanceName +"_" + Var2D + "_KITratio.pdf");
	  ...
	  hPurity->Draw("COLZ");
	  c_purstab->SaveAs("2D_purity_" + acceptanceName +"_" + Var2D + "_KITratio.pdf");
	  hStability->Draw("COLZ");
	  c_purstab->SaveAs("2D_stability_" + acceptanceName +"_" + Var2D + "_KITratio.pdf");
	  ...
	  c1->cd(3);
	  hTrue->Draw("COLZ");
	  c1->SaveAs("2D_data_comparison_"+acceptanceName+"_"+Var2D+"_KITratio.pdf");
	  ...
	  hResp->Draw("COLZ");
	  c_resp->SetLogz();
	  c_resp->SaveAs("2D_Response_" + acceptanceName + "_" + Var2D + "_KITratio.pdf");
	  ...
	  */
	  ...
	  c_afb->SaveAs("2D_AfbVs" + Var2D + "_unfolded_" + acceptanceName + "_KITratio.pdf");
	  ...
	  /*
		...
		c_mttu->SaveAs(Var2D + "_2D_unfolded_" + acceptanceName + "_KITratio.pdf");
