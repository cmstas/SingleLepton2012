...
void combinePlots_WH(char* sample = "TChiWH" , bool print = true , bool doFixups = false){
  ...
  if( TString(sample).Contains("TChiWH") ){
    label       = "pp #rightarrow #tilde{#chi}_{1}^{#pm}#tilde{#chi}_{2}^{0} #rightarrow W(l#nu)H(b#bar{b})#tilde{#chi}_{1}^{0}#tilde{#chi}_{1}^{0}";
    xaxismin    =  80.0;
    yaxismax    = 250.0;
  ...
  for( unsigned int i = 0 ; i < ncuts ; ++i ){
    hnsig[i]       = (TH2F*) file->Get(Form("hnsig_%i",i));
    ...
    if( doTruncation ){
      ...
      truncateHistAtDiagonal(hstaterr[i]    , dMCut);
  ...
  // output file for making datacards
  ofstream f_bestsrs;
  f_bestsrs.open("TChiWH_best_srs.txt");
  ...
  for( int xbin = 1 ; xbin <= hxsec_best->GetXaxis()->GetNbins() ; ++xbin ){
	...
	if( hxsec_exp[i]->GetBinContent(xbin,ybin) < min_exp_ul && nevents > NEVENTS_MIN ){
      ...
      // special cases: force signal regions
      if (forceSRs) {
	if (xbin == 7 && ybin == 1) { //150,1
	  best_ul = 0; 
	  std::cout << "forcing SR " << best_ul << " for 150,1" << std::endl;
	} else if (xbin == 13 && ybin == 7) { //300,150
	  best_ul = 0; 
	  std::cout << "forcing SR " << best_ul << " for 300,150" << std::endl;
	} else if (xbin == 14 && ybin == 4) { //325,75
	  best_ul = 3; 
	  std::cout << "forcing SR " << best_ul << " for 325,75" << std::endl;
	} else if (xbin == 18 && ybin == 13) { //425,300
	  best_ul = 0; 
	  std::cout << "forcing SR " << best_ul << " for 425,300" << std::endl;
	}
      }
      ...
      // output to file
      float mchargino = hxsec_best->GetXaxis()->GetBinCenter(xbin);
      float mlsp = hxsec_best->GetYaxis()->GetBinCenter(ybin);
      if ((mchargino - mlsp) < 130) {
      	if (mchargino == 125) mchargino = 130;
      	else mlsp = mchargino - 130;
      }
      f_bestsrs << mchargino << " " << mlsp << " " << best_ul << " " 
		<< hnsig[best_ul]->GetBinContent(xbin,ybin) << " " << 1.+hstaterr[best_ul]->GetBinContent(xbin,ybin) << std::endl;
    }
  }
  ...
  TFile* f = TFile::Open("c1n2_xsec.root");
  TH1F* refxsec = (TH1F*) f->Get("h_c1n2_xsec");
  ...
  TH2F* hR_exp   = exclusionContour(hxsec_best_exp   ,sample,doBDT,nsmooth,"nominal");
  TH2F* hR_expp1 = exclusionContour(hxsec_best_expp1 ,sample,doBDT,nsmooth,"nominal");
  TH2F* hR_expm1 = exclusionContour(hxsec_best_expm1 ,sample,doBDT,nsmooth,"nominal");
  TH2F* hR       = exclusionContour(hxsec_best       ,sample,doBDT,nsmooth,"nominal");
  TH2F* hR_obsp1 = exclusionContour(hxsec_best       ,sample,doBDT,nsmooth,"up");
  TH2F* hR_obsm1 = exclusionContour(hxsec_best       ,sample,doBDT,nsmooth,"down");
  ...
  hxsec_best_exp->GetXaxis()->SetRangeUser(0,512.5);
  TH2F* hdummy = new TH2F("hdummy","",100,0,512.5,100,0,430);
  hdummy->Reset();
  ...
  float xoffset = 0.47*(512.5-xaxismin)+xaxismin;
  float yoffset = 368.0;
  float length  = 0.092*(512.5-xaxismin);
  float yspace1 =     5;
  float yspace2 =    34;
  ...
  float xoffset = 0.53*(512.5-xaxismin)+xaxismin;
  float yoffset = 370.0;
  float length  = 0.05*(512.5-xaxismin);
  float yspace1 =     5;
  float yspace2 =    28;
  ...
  */
  ...
  if( print ){
    can1->Print(Form("plots/combinePlots_%s%s%s%s.pdf"               ,sample,suffix,BDTchar,nminchar));
